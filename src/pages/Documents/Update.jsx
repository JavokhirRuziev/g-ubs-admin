...
const Update = ({location, history, match}) => {
  ...
  return (
    <EntityContainer.One
      ...
      id={id}
      params={{
        include: "translations, files, categories",
      }}
    >
      {({item, isFetched}) => {
        return (
          <Spin spinning={!isFetched}>
            <div className="title-md mb-20 mt-14">{t('Изменить документ')}</div>
            ...
            <EntityForm.Main
              ...
              url={isOwn ? `/documents/${get(item, 'id')}` : '/documents'}
              updateData={isOwn}
              prependData={!isOwn}
              primaryKey="id"
              ...
              onSuccess={(data, resetForm) => {
                resetForm();
                history.push(`/documents?lang=${tabLang}`)
              }}
              fields={[
                {
                  name: "name",
                ...
                {
                  name: "category_id",
                  value: get(item, 'categories[0]', null),
                ...
                //   value: get(item, 'date_of_adoption') ? moment.unix(get(item, 'date_of_adoption')) : "",
                // }
              ]}
              params={{
