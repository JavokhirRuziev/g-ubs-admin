import React from 'react';

import {Spin} from 'antd';
import EntityForm from 'modules/entity/forms';
import Form from './Form';
import get from "lodash/get";

const Update = ({selected, showUpdateModal}) => {
    return (
        <EntityForm.Main
            method="put"
            entity="faq"
            name={`all`}
            url={`/faq/${get(selected, 'id')}`}
            primaryKey="id"
            normalizeData={data => data}
            id={get(selected, 'id')}
            onSuccess={(data, resetForm) => {
                resetForm();
                showUpdateModal(false)
            }}
            fields={[
                {name: "title_uz", required: true, value: get(selected, 'title_uz')},
                {name: "title_ru", required: true, value: get(selected, 'title_ru')},
                {name: "title_en", required: true, value: get(selected, 'title_en')},
                {name: "description_uz", required: true, value: get(selected, 'description_uz')},
                {name: "description_ru", required: true, value: get(selected, 'description_ru')},
                {name: "description_en", required: true, value: get(selected, 'description_en')},
                {name: "sort", value: get(selected, 'sort'), type: 'number'},
                {
                    name: "status",
                    value: !!get(selected, 'status'),
                    onSubmitValue: value => value ? 1 : 0
                }
            ]}
            updateData
        >
            {({isSubmitting, values, setFieldValue}) => {
                return (
                    <Spin spinning={isSubmitting}>

                        <Form {...{values, setFieldValue, isUpdate: true}}/>

                    </Spin>
                );
            }}
        </EntityForm.Main>
    );
};

export default Update;