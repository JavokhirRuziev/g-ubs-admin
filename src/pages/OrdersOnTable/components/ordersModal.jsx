...
const OrdersModal = ({ selected, showViewModal, setSelectedOrder }) => {
	...
	const totalTipPrice = items.reduce(
		(prev, curr) => prev + Number(curr.tip_price),
		0
	);
	const totalTotalPrice = items.reduce(
		(prev, curr) => prev + Number(curr.total_price),
		0
	);
	const totalTotalSum = items.reduce(
		(prev, curr) => prev + Number(curr.total_sum),
		0
	);
			...
			<table className="bordered">
				...
				<tbody>
						...
						<tr>
							...
							<td>
								{helpers.convertToReadable(
									get(item, "tip_price", 0)
								)}
							</td>
							<td>
								{helpers.convertToReadable(
									get(item, "total_price", 0)
								)}
							</td>
							<td>
								{helpers.convertToReadable(
									get(item, "total_sum", 0)
								)}
							</td>
							<td>
								<div
					...
					<tr>
						...
						<td>
							{totalTipPrice
								? helpers.convertToReadable(totalTipPrice)
								: "0"}
						</td>
						<td>
							{totalTotalPrice
								? helpers.convertToReadable(totalTotalPrice)
								: "0"}
						</td>
						<td>
							{totalTotalSum
								? helpers.convertToReadable(totalTotalSum)
								: "0"}
						</td>
						<td></td>
					</tr>
