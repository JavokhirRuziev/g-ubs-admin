        ...
        <div>
            ...
            {comment && (
                <div className="info-col mb-24">
                    ...
                    <div className="info-col__value">
                        <Tag color={'orange'} className="w-100p text-left">{comment}</Tag>
                    </div>
                </div>
            )}
            ...
            <div className="row">
                {isRejected && (
                    <div className="col-12 info-col mb-20">
                        <div className="info-col__label">Причина отклонение:</div>
                        <div className="info-col__value">
                            <Tag color={"red"} className="w-100p text-left">{rejectReason ? rejectReason : 'нет комментарии'}</Tag>
                        </div>
                    </div>
                )}
                ...
                <div className="col-4 info-col mb-20">
                    ...
                    <div className="info-col__value">{helpers.getOrderType(type)}</div>
                </div>
                <div className="col-4 info-col mb-20">
                    <div className="info-col__label">Номер стола:</div>
                    <div className="info-col__value">{table_number ? table_number : '-'}</div>
                </div>
                        ...
                        {payments.map(payment => (
                            <div className="info-col__value">
                                <div className="row mb-10">
                                    <div className="col-6">{helpers.getPaymentType(payment.payment_type)}:</div>
                                    <div className="col-6">{payment.amount.toLocaleString()}</div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                {address && (
                    <div className="col-12">
                        <div className="info-col address-block">
                            ...
                            <div className="info-col__value">
                                <a href={`https://yandex.ru/maps/?pt=${longitude},${latitude}&z=17&l=map`} target={"_blank"}>{address}</a>
                            </div>
                        </div>
                    </div>
                )}
            ...
            <div className="order-goods">
                ...
                <table className="bordered">
                    ...
                    <tbody>
                    {goods.map(item => {
                        const a = get(item, 'price',1) * get(item, 'quantity',1)
                            ...
                            <tr key={item.id}>
                                <td style={{width:'72px'}}>
                                    <Avatar isProduct
                                            isRectangle
                                            size={"sm"}
                                            image={get(item, 'dish.file.thumbnails.small.src')}
                                            bigImage={get(item, 'dish.file.thumbnails.low.src')}
                                    />
                                </td>
                                <td>{get(item, 'dish.translate.name')}</td>
                                <td>{quantity ? helpers.convertToReadable(quantity) : ''}</td>
                                <td>{helpers.convertToReadable(get(item, 'price'))}</td>
                                <td>{a ? helpers.convertToReadable(a) : 0}</td>
                            </tr>
                        )
                    })}
                    <tr>
                        ...
                        <td>
                            {products_sum ? helpers.convertToReadable(products_sum) : '-'}
                        </td>
                    </tr>
                    <tr>
                        <td colSpan={"4"}>
                        ...
                        <td>
                            {delivery_price ? helpers.convertToReadable(delivery_price) : 0}
                        </td>
                    </tr>
                    <tr>
                        <td colSpan={"4"}>
                        ...
                        <td>
                            {total_sum ? helpers.convertToReadable(total_sum) : '-'}
                        </td>
                    </tr>
                    </tbody>
                </table>
