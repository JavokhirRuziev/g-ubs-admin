...
import UpdateFile from "./components/FileUpdate";
...
export default function index({ location, history, match }) {
	...
	const [updateFileModal, showUpdateFileModal] = useState(false);
	...
	const changeTab = value => {
		history.push(`/template?lang=${value}`);
	};
	...
	const openFileEdit = value => {
		setSelected(value);
		showUpdateFileModal(true);
	};
	...
	return (
		<>
			<div className="d-flex justify-content-between align-items-center mb-20">
				<div className="title-md">{t("Шаблоны")}</div>
			</div>
			<Modal
				visible={createModal}
			...
			<Modal
				visible={updateFileModal}
				onOk={() => showUpdateFileModal(true)}
				onCancel={() => showUpdateFileModal(false)}
				footer={null}
				centered
				width={430}
				destroyOnClose>
				<UpdateFile {...{ selected, showUpdateFileModal }} />
			</Modal>
