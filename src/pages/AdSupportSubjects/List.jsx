import React, {useEffect, useState} from 'react';

import {Table, Board} from "components";
import {Button, Pagination, Spin, Modal, notification} from "antd";
import EntityContainer from 'modules/entity/containers';
import Create from "./components/Create";
import Update from "./components/Update";
import Actions from "modules/entity/actions";
    import ModuleActions from 'modules/entity/actions';

import {useTranslation} from "react-i18next";
import get from "lodash/get";
import {useDispatch} from "react-redux";

const List = ({match}) => {
    const [createModal, showCreateModal] = useState(false);
    const [updateModal, showUpdateModal] = useState(false);
    const [selected, setSelected] = useState(null);
    const [parent, setParent] = useState(null);
    const [page, setPage] = useState(1);
    const {parent_id} = match.params;

    const {t} = useTranslation();
    const dispatch = useDispatch();

    useEffect(() => {
        dispatch(ModuleActions.LoadDefault.request({
            url:`/advertising/${parent_id}`,
            cb: {
                success: (data) => setParent(data),
                error: () => {}
            }
        }))
    }, [])

    const openEditModal = value => {
        setSelected(value);
        showUpdateModal(true);
    };

    const onDeleteHandler = menuId => {
        Modal.confirm({
            title: t("Вы действительно хотите удалить?"),
            okText: t("да"),
            okType: "danger",
            cancelText: t("нет"),
            confirmLoading: true,
            onOk: () => deleteAction(menuId),
        });
    };

    const deleteAction = id => {
        dispatch(Actions.Form.request({
            method: 'delete',
            entity: "subjects",
            name: `subjects-${parent_id}`,
            id: id,
            url: `/advertising-subjects/${id}`,
            deleteData: true,
            cb: {
                success: () => {
                    notification["success"]({
                        message: t("Успешно удалена"),
                        duration: 2
                    });
                },
                error: () => {
                    notification["error"]({
                        message: t("Что-то пошло не так"),
                        duration: 2
                    });
                },
                finally: () => {
                }
            }
        }))
    };

    return (
        <>
            <Modal
                visible={createModal}
                onOk={() => showCreateModal(true)}
                onCancel={() => showCreateModal(false)}
                footer={null}
                centered
                width={700}
                destroyOnClose
            >
                <Create {...{showCreateModal, parent_id}}/>
            </Modal>
            <Modal
                visible={updateModal}
                onOk={() => showUpdateModal(true)}
                onCancel={() => showUpdateModal(false)}
                footer={null}
                centered
                width={700}
                destroyOnClose
            >
                <Update {...{selected, showUpdateModal, parent_id}}/>
            </Modal>

            <div className="d-flex justify-content-between align-items-center mb-20">
                <div className="title-md">{get(parent, 'title')}</div>
                <Button
                    type="primary"
                    size="large"
                    className="fs-14 fw-300 ml-10"
                    htmlType="button"
                    onClick={() => showCreateModal(true)}
                >Добавить</Button>
            </div>

            <Board className="border-none">
                <EntityContainer.All
                    entity="subjects"
                    name={`subjects-${parent_id}`}
                    url="/advertising-subjects"
                    params={{
                        sort: 'sort',
                        limit: 50,
                        page,
                        filter: {parent_id: parent_id}
                    }}
                >
                    {({items, isFetched, meta}) => {
                        return (
                            <Spin spinning={!isFetched}>
                                <div className="default-table pad-15">
                                    <Table
                                        hasEdit={true}
                                        hasDelete={true}
                                        rowKey="id"
                                        onEdit={value => openEditModal(value)}
                                        onDelete={value => onDeleteHandler(value.id)}
                                        columns={[
                                            {
                                                title: t("ID"),
                                                dataIndex: "id",
                                                className: 'w-50',
                                                render: value => <div className="divider-wrapper">{value}</div>
                                            },
                                            {
                                                title: t("Сортировка"),
                                                dataIndex: "sort",
                                                className: 'w-100',
                                                render: value => <div className="divider-wrapper">{value}</div>
                                            },
                                            {
                                                title: t("Название"),
                                                dataIndex: `title`,
                                                render: value => <div className="divider-wrapper">{value}</div>
                                            }
                                        ]}
                                        dataSource={items}
                                    />
                                </div>
                                {meta && meta.perPage && (
                                    <div className="pad-15 d-flex justify-content-end">
                                        <Pagination
                                            current={meta.currentPage}
                                            pageSize={meta.perPage}
                                            total={meta.totalCount}
                                            onChange={newPage => setPage(newPage)}
                                        />
                                    </div>
                                )}
                            </Spin>
                        );
                    }}
                </EntityContainer.All>
            </Board>
        </>
    );
};

export default List;